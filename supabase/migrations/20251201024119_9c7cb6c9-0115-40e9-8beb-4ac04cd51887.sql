...
-- 1. Adicionar SET search_path na função send_daily_equipment_report()
CREATE OR REPLACE FUNCTION public.send_daily_equipment_report()
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  RAISE NOTICE 'Daily equipment report sent successfully at %', now();
END;
$function$;

-- 2. Comentário explicativo: A view v_withdrawals_with_remaining usa security_invoker
-- que é o comportamento correto e seguro. O linter pode estar detectando
-- falsamente. Verificar manualmente se necessário.