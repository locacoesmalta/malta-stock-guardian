-- Adicionar coluna detalhes_evento se não existir
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'patrimonio_historico' 
    AND column_name = 'detalhes_evento'
  ) THEN
    ALTER TABLE public.patrimonio_historico ADD COLUMN detalhes_evento TEXT;
  END IF;
END $$;
